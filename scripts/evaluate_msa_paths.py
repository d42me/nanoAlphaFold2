"""Separate cluster-MSA and extra-MSA contributions at a fixed depth."""
...
CONDITIONS = ("query_only", "cluster_only", "extra_only", "both_paths")
...
def path_batch(features, depth, seed, condition):
    ...
    if condition in ("query_only", "extra_only"):
        cluster_rows = torch.tensor([0])
    if condition in ("query_only", "cluster_only"):
        extra_rows = torch.empty(0, dtype=torch.long)
        ...
        for condition in CONDITIONS:
            for seed in args.seeds:
                for protein in proteins:
                    ...
                    print(
                        f"{condition:12s} seed={seed} {protein:18s} {rmsd:6.2f} Å",
                        flush=True,
                    )
