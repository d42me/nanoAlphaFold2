"""Separate query sequence, MSA profile, and extra-MSA contributions."""
...
def profile_path_batch(features, depth, seed, condition):
    sequences = features["msa_aatype"]
    deletions = features["deletion_matrix"]
    full_profile = features["profile"]
    query_profile = sequences[0]
    ...
    profile = (
        full_profile
        if condition in ("profile_only", "extra_plus_profile")
        else query_profile
    )
        ...
        for condition in CONDITIONS:
            for seed in args.seeds:
                for protein in proteins:
                    ...
                    print(
                        f"{condition:18s} seed={seed} {protein:18s} {rmsd:6.2f} Å",
                        flush=True,
                    )
